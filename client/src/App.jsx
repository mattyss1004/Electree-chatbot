...
SÍTĚ:
- ES: Electree Solar (nové instalace, produkty ES, servis ES, dotace ES, NZÚ 2026, bezúročný úvěr).
- TRANSFER: Přechod Acetex→ES (insolvence Acetexu, co ES přebírá, kontakty pro klienty Acetexu, kompenzace Výzva 1/2026).
- ACETEX: Technická databáze Acetexu (existující instalace, produkty GoodWe, Pylontech, Wattsonic, AEG, Viessmann, Stiebel Eltron, JA Solar, Luxen, SEMS portal, chybové kódy, technické problémy).

PRAVIDLA:
1. Pokud se dotaz týká "Acetexu", "přechodu", "insolvence" nebo "kompenzace", VŽDY zahrň TRANSFER.
2. Pokud se dotaz týká konkrétních značek (GoodWe, Pylontech, Wattsonic, AEG, Viessmann, Stiebel, JA Solar, Luxen) nebo technických chyb (E04, monitoring, SEMS), VŽDY zahrň ACETEX.
3. Síť ES zahrň POUZE pokud se uživatel ptá na NOVÉ produkty Electree Solar, NOVÉ dotace (mimo kompenzace Acetexu) nebo obecné služby ES.
4. Pokud dotaz obsahuje témata z VÍCE sítí, vrať VŠECHNY relevantní.
5. Odpovídej POUZE jako čistý JSON.
  ...
  const send=useCallback(async(text)=>{
    ...
    try{
      ...
      try{ 
        ...
        nets=j.networks||["ES"];
        // Heuristic fallback: if user mentions Acetex but router missed it
        const low = text.toLowerCase();
        if (low.includes("acetex") && !nets.includes("TRANSFER")) nets.push("TRANSFER");
        const techTerms = ["goodwe", "pylontech", "wattsonic", "aeg", "viessmann", "stiebel", "ja solar", "luxen", "sems", "chyba", "error"];
        if (techTerms.some(t => low.includes(t)) && !nets.includes("ACETEX")) nets.push("ACETEX");
      }catch{}
      setActiveNets(nets);
