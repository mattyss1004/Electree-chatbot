...
KOMPENZACE NZÚ 2026 – VÝZVA č. 1/2026 (NPŽP–NPO–č. 1/2026):
Oficiální název: Výzva k financování žádostí o podporu na projektovou přípravu v rámci programu Nová zelená úsporám z prostředků Národního plánu obnovy.
Kdo má nárok: Žadatelé, kteří připravili odborný posudek k žádosti o dotaci NZÚ (oblast A – zateplení nebo oblast B – novostavba) v rámci výzev HOUSENERG č. 1/2025 (RD), č. 6/2023 (BD – SVJ a bytová družstva) nebo č. 8/2023 (fyzické a právnické osoby), ale nestihli podat žádost kvůli vyčerpání alokace.
Výše podpory:
- Rodinné domy (RD): až 50 000 Kč
- Bytové domy (BD): až 100 000 Kč
Alokace výzvy: 200 mil. Kč celkem.
Termíny: Příjem žádostí od 25. 3. 2026 v 10:00 do 15. 4. 2026 v 15:00. POZOR: výzva je časově omezená.
Co je podporovaným výdajem: Náklady na zpracování odborného posudku (energetický specialista, PENB). DPH není způsobilým výdajem. Výdaj musí být uhrazen v období od 1. 9. 2025 do 15. 4. 2026 a doložen fakturou.
Jak podat žádost: Elektronicky přes AIS SFŽP ČR na https://zadosti.sfzp.cz/. Nutná bankovní identita nebo NIA (datová schránka, eObčanka). Žádosti se administrují průběžně v pořadí podání.
Povinné dokumenty:
1. Doklad o vlastnictví bankovního účtu
2. Odborný posudek (PENB registrovaný v systému ENEX od 1. 9. 2025)
3. Faktura + doklad o zaplacení
4. Případně čestné prohlášení de minimis (pro firmy)
Důležité podmínky: Na stejný odborný posudek nelze čerpat podporu z více programů současně. Podpora se nevztahuje na posudky, které jsou předmětem jiné administrované žádosti.
Kontakt a informace: www.novazelenausporam.cz, krajská pracoviště SFŽP ČR.
Electree Solar vám může pomoci s novým řešením FVE/TČ a případnou novou žádostí o dotaci.`;
...
const ROUTER_PROMPT = `Jsi expertní router pro inteligentní chatbot Electree Solar. Tvým úkolem je provést hloubkovou analýzu uživatelského dotazu a určit, které informační sítě jsou nezbytné pro přesnou odpověď.

DOSTUPNÉ SÍTĚ:
1. ES (Electree Solar): Nové produkty, aktuální dotace NZÚ 2026, bezúročné úvěry, nové instalace, servisní balíčky ES, obecné informace o firmě.
2. TRANSFER (Přechod Acetex): Vše kolem insolvence Acetexu, proces přebírání klientů pod ES, kompenzační výzva č. 1/2026, kontakty pro bývalé klienty.
3. ACETEX (Technická databáze): Specifický hardware (GoodWe, Pylontech, Wattsonic, AEG, Viessmann atd.), chybové kódy (E04...), monitoring (SEMS), technické parametry starších instalací.

POSTUP ANALÝZY (Chain-of-Thought):
1. Identifikuj klíčové entity (firmy, značky, dotační programy).
2. Urči záměr uživatele (řeší problém z minulosti, chce něco nového, nebo obojí?).
3. Vyhodnoť překryvy: Pokud se ptá na "dotaci pro klienta Acetexu", potřebuješ TRANSFER (pro kontext klienta) i ES (pro informace o nových dotacích).
4. Rozhodni o sítích: Buď inkluzivní u komplexních dotazů.

VÝSTUP: Pouze čistý JSON.
Formát: {"reasoning":"stručné zdůvodnění v češtině", "networks":["SÍŤ1", "SÍŤ2"]}`;
...
export default function App(){
  ...
  const send=useCallback(async(text)=>{
    ...
    try{
      ...
      let nets=["ES"];
      try{ 
        const j=JSON.parse(rr.replace(/```json|```/g,"").trim()); 
        nets=j.networks||["ES"];
        
        // DEEP SEMANTIC HEURISTICS (Czech)
        const low = text.toLowerCase();
        const transferKeywords = ["acetex", "insolvence", "přechod", "přebírání", "zkrachoval", "kompenzace", "výzva 1/2026", "výzva č. 1/2026", "původní projekt"];
        const techKeywords = ["goodwe", "pylontech", "wattsonic", "aeg", "viessmann", "stiebel", "ja solar", "luxen", "sems", "chyba", "error", "firmware", "monitoring", "ean", "wattrouter", "iboost"];
        const esKeywords = ["nová", "zelená", "úsporám", "nzu", "úvěr", "bezúročný", "nabídka", "cena", "kolik stojí", "konzultace", "servisní balíček"];

        // Force networks based on bulletproof keywords
        if (transferKeywords.some(k => low.includes(k)) && !nets.includes("TRANSFER")) nets.push("TRANSFER");
        if (techKeywords.some(k => low.includes(k)) && !nets.includes("ACETEX")) nets.push("ACETEX");
        if (esKeywords.some(k => low.includes(k)) && !nets.includes("ES")) nets.push("ES");

        // Optimization: If it's a legacy technical/transition query, remove ES unless explicitly asked for new things
        if ((nets.includes("TRANSFER") || nets.includes("ACETEX")) && nets.includes("ES") && nets.length > 1) {
          const explicitNewSales = ["mám zájem", "chci koupit", "poptávka", "nabídka", "kolik stojí", "novou fve", "nové tč"];
          if (!explicitNewSales.some(k => low.includes(k))) {
            nets = nets.filter(n => n !== "ES");
          }
        }
        
        if (nets.length === 0) nets = ["ES"];
      }catch{
        nets = ["ES"];
      }
      setActiveNets(nets);
